...
const ElementEditor = ({
  ...
  const isMobile = useMediaQuery(theme.breakpoints.down("md")); // md breakpoint for tablets/desktop
  const isGroup = selectedElement?.type === "group";
  const editor = useElementEditor(selectedElement, onUpdateElement, formElements);
  ...
  if (!selectedElement) {
    if (isMobile) {
      return null; // Don't show empty state on mobile - handled by parent
    }
    return (
  ...
  // Desktop: Inline panel (existing layout)
  if (!isMobile || compact) {
    return (
      <Box p={compact ? 2 : 4} height="100%" overflow="auto" bgcolor="background.paper">
        {renderContent()}
