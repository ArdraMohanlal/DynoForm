...
//         // If it's a group and fields exist
...
//       deleteElement: (id) =>
//         set((state) => {
//           const topLevelFiltered = state.formElements.filter(el => el.id !== id);
...
export const useFormStore = create(
  persist(
    (set, get) => ({
      ...
      // NEW: load a full template into the builder
      loadTemplate: (templateElements) =>
        set(() => ({
          formElements: templateElements,
          // ensure following adds use higher ids than any in the template
          nextId:
